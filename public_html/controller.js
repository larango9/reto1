    ...
    $.ajax({
            ...
            success: function(data) {
                ...
                for(var i = 0; i < data.length; i++){
                    str += '<div><img width=50 height=50 src="imagesUploaded/'+ data[i].name +'" /><a target="_blank" href="imagesUploaded/'+ data[i].name +'">'+ data[i].name +'</a></div>';
                }
                $('#listFiles').html(str);
            }
     });
...
function closeSession(){
    $.ajax({
        ...
        success: function(data) {
            if(data.session == true){
                $('#menuRegister').fadeOut(0);
                $('#menuInit').fadeIn(0);
                $('#uploadImage').fadeOut(0);
            }else{
